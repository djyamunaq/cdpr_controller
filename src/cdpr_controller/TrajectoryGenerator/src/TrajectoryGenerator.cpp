...
TrajectoryGenerator::TrajectoryGenerator(Vector3d pis, Vector3d pfs, double Dt, double ti) {
    this->Dt = Dt;
    this->ti = ti;
    this->polX = this->getPol(pis(0), pfs(0));
    this->polY = this->getPol(pis(1), pfs(1));
}
...
Eigen::Vector3d TrajectoryGenerator::X(double t){
  return Eigen::Vector3d(this->p(t, this->polX), this->p(t, this->polY), 1);
}

Eigen::Vector3d TrajectoryGenerator::dX(double t){
  return Eigen::Vector3d(this->dp(t, this->polX), this->dp(t, this->polY), 1);
}
